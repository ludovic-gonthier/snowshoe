  ...
  return {
    organizations: function (user) {
      return new Promise(function(resolve, reject) {
        ...
        request.call(user.organizations_url + '?per_page=100')
          .then(notifyRate)
    ...
    pulls: function (repository, callback) {
      return new Promise(function(resolve, reject) {
        request.call(repository.pulls_url.replace(regexp.number, '') + '?per_page=100')
          .then(notifyRate)
    ...
    issues: function (repository) {
      return new Promise(function (resolve, reject) {
        request.call(repository.issues_url.replace(regexp.number, '') + '?per_page=100')
          .then(notifyRate)
    ...
    statuses: function (pulls) {
      return new Promise(function (resolve, reject) {
        async.each(pulls, function (pull, callback) {
          request.call(pull.statuses_url.replace(regexp.sha, '') + '?per_page=100')
            .then(notifyRate)
